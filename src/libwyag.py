...
argsp.add_argument
默认传入的字符串是 default="."
dest="write"我们可以通过args.write去访问我们传入的值,
当然我们传入的第一个参数如果是'-v'或者'--value',
那么不可以加dest(否则会报错),可通过v/value访问传入的参数
metavar是用于说明文档的变量名
nargs变量数
type是变量类型
help说明文档
...
def kvlm_parse(raw, start=0, dct=None):
    ...
    #{ b"author":b"adl",b"nick_name":["adlternative","hzn"] }
    #如果键已经存在字典中
    #若该键对应的值已经是一个列表了,我们就将value添加列表尾部
    #若该键对应的值已经是一个元素,那我们将创建一个列表去再将value添加列表尾部
    #否则将键添加到字典就ok
    if key in dct:
...
def tree_parse_one(raw, start=0):
   """ 40000 keysL��@[�X��Nn�f\�!Z40000 paths��$�C}��~�'�Ԣ�y|��% """
    """　找空格 """
    x = raw.find(b' ', start)
...
def cmd_checkout(args):
    ...
    """貌似它实现的这个checkout必须用到另外一个空白目录下面  """
...
def ref_resolve(repo, ref):
  """
    如果是个引用,去找真正的文件,读取其中的内容--->最后是一个hash
    cat .git/refs/heads/master
    75f52c20ad6ed23c37534fe06d2592b42e8408ea
   """
...
def object_resolve(repo, name):
    ...
    """ 如果是一个引用,去找真正文件中的hash """
    if name == "HEAD":
        return [ ref_resolve(repo, "HEAD") ]

    """ 如果是一个hash,去找真hash(by path find) """
