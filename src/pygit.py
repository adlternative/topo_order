...
def find_object(sha1_prefix):
    ''' ok  '''
    ''' return with 文件在gitobject数据库中的路径 '''
...
def read_index():
    ...
    while i + 62 < len(entry_data):
        ''' -----------------entry---------------------- '''
        ''' i=0- -  fields- - -(f_e)62--path--(p_e)'''
        fields_end = i + 62
        fields = struct.unpack('!LLLLLLLLLL20sH', entry_data[i:fields_end])
        # 因为path不定长得通过寻找 0 的方式去获得path
...
def write_index(entries):
    ...
    for entry in entries:
        ...
        packed_entry = entry_head + path + b'\x00' * (length - 62 - len(path))  #as you see,多余补0
...
def add(paths):
    ...
    for path in paths:
        ...
        assert flags < (1 << 12)# pathlen < 2^12 =4096
        # as you see add to index
        # the flag will turn to one in bitmap by len(path.encode())
        entry = IndexEntry(
